...
\NEW{Abil}{\aDetectSavedState}{
  ...
  \s\MYeffect	{If you have a $\beta$ score, tell me your $\beta$ score, otherwise tell me you have no $\beta$ score.} % TODO: add beta score=2 to child and beta score=1 to child's mother
  }
...
\NEW{Abil}{\aDebug}{
  \s\MYname	{Debug}
  \s\MYtext {You may return someone to their maximum $\Omega$ score, but you must possess and consume a widget to do so. You can also pickup widgets.}
  \s\MYeffect {Bug-fixing. My favorite thing.}
}

\NEW{Abil}{\aCollectBlood}{
  \s\MYname	{Collect blood sample}
  \s\MYtext {if you spend 1 minute with a syringe item, a vial item, and a willing or helpless character, take a blank item sheet and write ``X's blood'' on it.}
  \s\MYeffect	{Field xenobiology is your specialty.}
}
