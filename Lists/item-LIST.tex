...
\NEW{Item}{\isyringe}{
  ...
  \s\MYtext	{An expensive personal computer. Can be broken with an attack, opening the packet beneath.}
   \s\MYcontents{ Cross out the "Laptop" card and make a nonbulky 'Processor' item card.}
    ...
    \NEW{Item}{\iDogtags}{
  \s\MYname	{Military dogtags}
  \s\MYnumber	{0192}
  \s\MYtext	{A set of military dogtags on a ball chain. The name on them reads \cGroomA{}}
  }    %% in the groom's room
...
%  }
  ...
  \s\MYnumber	{0294}
  \s\MYtext	{These are some basic sketches and blueprints for a few types of advanced military flight technology, such as airplanes and missiles. If you are the original owner, you jotted these down because you suspected the locals might find it interesting, but they are not overly revealing.}
  }
...
\NEW{Item}{\iFlowers}{
  ...
  \s\MYnumber	{0156}
  \s\MYtext	{Maybe thirty, thirty-five years old. Creased and worn, like it has been carried around a lot. The picture shows a man and a small boy. Come to think of it, that boy a bit like \cKid{}, but that's probably just a coincidence\ldots{} }
  }  
...
\NEW{Item}{\iBunnyRabbit}{
  ...
  \s\MYnumber {0148}
    \bulky{1}
  \s\MYtext	{A stuffed toy rabbit with big floppy ears. \bf{Unstashable}}
  }
...
\NEW{Item}{\iDrug}{
	\s\MYname	{Bite}
  \s\MYnumber	{0230}
  \s\MYtext	{A package containing several doses of Anaphmonine, which goes by the street name `Bite'.}
  }
  
\NEW{Item}{\iStocks}{
	\s\MYname	{Company Stock}
  \s\MYnumber	{0233}
  \s\MYtext	{A paper denoting ownership of about \$250,000 worth of stock in the ``Vision'' company}
  }
